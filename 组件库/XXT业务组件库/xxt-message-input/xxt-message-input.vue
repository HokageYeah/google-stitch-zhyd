<template>
  <view class="input-container" :class="{ 'host-managed-unimp-app': hostManagedUnimpApp }">
    <view class="input-box">
      ...
      >
        ...
        <!-- #endif -->
        <!-- #ifndef MP-WEIXIN -->
        {{ isRecording ? '松开 结束' : hostManagedUnimpApp ? '按住 说话' : '按住 说话（请先插入耳机再说话）' }}
        <!-- #endif -->
      </view>
    </view>
  </view>
...
import { $cdn, appModuleConfig, isHostManagedUnimpApp } from '@/uni-module-common/config';
...
// 缓存宿主管理 App 命中结果，模板样式和留言参数分流共用同一判断。
...
const buildSendMessageParams = (
  ...
  keepMasterAppRoomBranch = false
) => {
  // 家校留言默认走学校/项目维度。众享智阅 App 当前入口 roomId=0，不是有效房间号，不能切到 hostType=2。
  ...
  // #ifdef APP-PLUS
  // 语音在 master 中已有 App 房间分支，文本没有；用 keepMasterAppRoomBranch 精确保留旧行为。
  const shouldUseRoomHost =
    (!hostManagedUnimpApp && keepMasterAppRoomBranch) ||
    (hostManagedUnimpApp && props.roomId > 0);
  if (shouldUseRoomHost) {
    params = {
      hostType: 2,
      roomId: props.roomId,
      ...data
...
const linkSure = async (content: string) => {
  ...
  if (content.trim()) {
    try {
      uniShowToast('发送中...');
      const sendRes: any = await sendLeaveMessage(
        props.sendUrl,
...
.input-container.host-managed-unimp-app {
  background-color: #fff;
}
.input-box {
...
.host-managed-unimp-app .input-box {
  height: 40px;
  .input-box-voice-img {
    width: 24px;
    height: 24px;
