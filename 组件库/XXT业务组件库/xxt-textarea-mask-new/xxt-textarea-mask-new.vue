  ...
  >
    <view class="textarea-container">
      ...
      <tui-form-button
        ...
        width="80rpx"
        height="60rpx"
        @touchstart="submitTouching = true"
        @click="linkSure"
        >确定</tui-form-button
      >
    </view>
...
// 宿主管理 App 由容器处理键盘顶起，textarea 自身不再 adjust-position，避免遮罩跟随键盘二次跳动。
const hostManagedUnimpApp = isHostManagedUnimpApp();
const textareaAdjustPosition = ref(!hostManagedUnimpApp);
const platform = ref('');
const isAndroid = ref(false);
const submitTouching = ref(false);
const textareaBottom = computed(() => {
  if (!popupShow.value || keyboardHeight.value <= 0) {
    return '-100%';
  }
...
// 隐藏输入框
const hiddenPopup = () => {
  if (!popupShow.value && !focus.value) {
    return;
  }
  popupShow.value = false;
  focus.value = false;
  keyboardHeight.value = 0;
  if (hostManagedUnimpApp) {
    // App 端主动隐藏键盘，保证遮罩关闭和原生键盘收起保持同一节奏。
    uni.hideKeyboard();
  }
  emits('linkCancel');
};
...
const callBackBlur = (e: any) => {
  ...
  if (hostManagedUnimpApp && !submitTouching.value) {
    // 非提交触发的失焦通常来自点击遮罩或系统收键盘，直接收口弹层避免残留半屏输入框。
    hiddenPopup();
  }
};
...
const keyboardheightchange = (e: any) => {
  ...
  if (e.height <= 0 && !clear) {
    if (hostManagedUnimpApp) {
      // 宿主管理 App 不延迟关闭，避免键盘高度归零后遮罩短暂闪烁。
      hiddenPopup();
      return;
    }
