<template>
  <slot
    ...
    :is-show-add-button="isShowImageAddButton"
    :choose="() => iconClick(1)"
    :delete-file="deleteFiles"
    :remove-image="removeImage"
    :update-image-list="updateImgAry"
  >
    <XXTImgList
  ...
  <slot
    name="videos"
    :video-list="videoAry"
    :max-video-number="maxVideoNumber"
    :is-nine-grid="isNineGrid"
    :choose="() => iconClick(3)"
    :delete-file="deleteFiles"
    :remove-video="removeVideo"
    :update-video-list="updateVideoAry"
  >
    <XXTVideoList
  ...
  <slot
    ...
    :update-audio-list="updateAudioAry"
  >
    <XXTAudioPlayer
  ...
  <slot
    ...
    :choose-file="() => iconClick(4)"
    :choose-link="() => iconClick(5)"
    :delete-file="deleteFiles"
    :remove-file="removeFile"
    :update-file-list="updateFileAry"
  >
    <XXTFileDocument
  ...
  <slot
    name="actions"
    :visible="computedHideChooseFile"
    :actions="computedIconBtns"
    :file-must-types="fileMustTypes"
    :choose="iconClick"
  >
...
import XXTIconButton from '../xxt-file-submit/xxt-icon-button.vue';
import XXTImgList from '../xxt-file-submit/xxt-img-list.vue';
import XXTVideoList from '../xxt-file-submit/xxt-video-list.vue';
// import XXTVideoRenderjsList from '../xxt-file-submit/xxt-video-renderjs-list.vue';
import XXTRecordingMask from '../xxt-file-submit/xxt-recording-mask.vue';
import XXTFileDocument from '../xxt-file-submit/xxt-file-document.vue';
import XXTLinkMask from '../xxt-file-submit/xxt-link-mask.vue';
...
import type {
  ...
  imageObjType,
  videoType,
} from '../xxt-file-submit/xxtFileType';
import { iconBtns } from '../xxt-file-submit/xxtFileCommonData';
import { getData } from '../xxt-file-submit/xxtFilesManager';
// import { base64DataDraw } from '../xxt-file-submit/xxtFilesManageDraw';
import {
  dealWithFile,
  dealWithImage,
  dealWithVideo,
  ...
  useDiffMediaFile,
  useShowActionSheet,
} from '../xxt-file-submit/useChooseMedia';
import { ResolutionModeEnum, VideoQualityEnum } from '../xxt-file-submit/videoCompressEnum';
import {
  natToUniEditedImg,
...
const removeImage = (fileObj: imageObjType) => {
  updateImgAry(imageAry.value.filter((item: imageObjType) => item !== fileObj));
  deleteFiles(1, fileObj);
};
const removeVideo = (fileObj: videoType) => {
  updateVideoAry(videoAry.value.filter((item: videoType) => item !== fileObj));
  deleteFiles(3, fileObj);
};
const removeAudio = (fileObj: audioType) => {
  updateAudioAry(audioAry.value.filter((item: audioType) => item !== fileObj));
  deleteFiles(2, fileObj);
};
const removeFile = (fileObj: fileType) => {
  updateFileAry(fileAry.value.filter((item: fileType) => item !== fileObj));
  deleteFiles(fileObj.fileType === 0 ? 5 : 4, fileObj);
};
onLoad(() => {
