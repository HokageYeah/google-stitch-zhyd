...
// 统一附件数量超限提示；平台适用范围由各入口的条件编译控制。
const showLimitToast = (limit: number, fileTypeName: '音频文件' | '视频文件' | '文件' | '链接') => {
  uniShowToast(`您最多可以上传${limit}个${fileTypeName}`);
};

const hasSelectedSingleLink = () => fileAry.value.some((item: fileType) => item.fileType === 0);
const chooseAlbum = () => {
...
const chooseAppVideo = () => {
  if (videoAry.value.length >= props.maxVideoNumber) {
    showLimitToast(props.maxVideoNumber, '视频文件');
    return;
  }
...
const chooseAppFiles = () => {
  if (fileAry.value.filter((item: fileType) => item.fileType === 1).length >= props.maxFileNumber) {
    showLimitToast(props.maxFileNumber, '文件');
    return;
  }
...
const iconClick = async (type: number) => {
  ...
  switch (type) {
    ...
    case 2:
      {
        ...
        if (audioAry.value.length >= props.maxRecordNumber) {
          showLimitToast(props.maxRecordNumber, '音频文件');
          return;
        }
        const audioObj = {
    ...
    case 3:
      // 视频选择
      // #ifdef APP-PLUS
      // App 视频可来自原生相册或微信小程序，统一在入口处拦截，避免两个入口各自放大可选数量。
      if (videoAry.value.length >= props.maxVideoNumber) {
        showLimitToast(props.maxVideoNumber, '视频文件');
        return;
      }
    ...
    case 4:
      {
        // #ifdef APP-PLUS
        // 文件入口包含“应用内选择”和“微信聊天选择”，先按已选文件数收口再弹选择面板。
        if (
          fileAry.value.filter((item: fileType) => item.fileType === 1).length >=
          props.maxFileNumber
        ) {
          showLimitToast(props.maxFileNumber, '文件');
          return;
