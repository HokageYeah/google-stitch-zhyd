<template>
  <view class="list-container" :class="{ 'host-managed-unimp-app': hostManagedUnimpApp }">
    <view
      v-for="(item, index) in roomMessageListStore.getMessageList(roomId)"
      :key="item.messageId"
      class="message-item"
      :class="{ 'host-managed-unimp-app': hostManagedUnimpApp }"
...
import { appModuleConfig, isHostManagedUnimpApp } from '@/uni-module-common/config/';
...
// 缓存宿主管理 App 命中结果，模板样式和列表参数分流共用同一判断。
...
// 默认查询学校/项目维度留言。众享智阅 App 的家校留言入口 roomId=0，应继续使用 hostType=1 + hostId。
...
// #ifdef APP-PLUS
// 非宿主管理 App 沿用 master 的 App 房间查询；宿主管理 App 只有真实 roomId 才切换到房间维度。
if (!hostManagedUnimpApp || props.roomId > 0) {
...
.list-container.host-managed-unimp-app {
  background-color: #f5f5f5;
}
.message-item {
  /* #ifndef MP-WEIXIN */
...
.message-item.host-managed-unimp-app {
  background-color: #f5f5f5;
}
</style>
