...
const tabbar = computed(() => {
  ...
  // 众享智阅没有单独配置 basicPre/basicAI，宿主管理 App 回退到 basic，避免底部导航为空。
  const template = appConfig.value.template || {};
  const basicTabBar = template.basic?.tabBar || null;
  const basicPreTabBar = isHostManagedUnimpApp()
    ? template.basicPre?.tabBar || basicTabBar
    : template.basicPre?.tabBar;
  const basicAITabBar = isHostManagedUnimpApp()
    ? template.basicAI?.tabBar || basicTabBar
    : template.basicAI?.tabBar;
  let tab = null;
  // #ifdef APP-PLUS
  if (
