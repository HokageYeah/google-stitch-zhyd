  ...
  <view class="u-tabbar">
    <view
      ...
      :class="[
        ...
        shouldUseHostManagedSafeBottom && 'u-tabbar--host-managed-safe-bottom',
        { 'mid-tabbar': midTabBar }
      ]"
      :style="[
        tabbarStyle,
        {
          borderTop:
            value !== '/pages/friends-circle-home/friends-circle-home'
              ? '1rpx solid rgba(0, 0, 0, 0.1)'
              : '1rpx solid rgba(255, 255, 255)'
        }
      ]"
...
export default {
  ...
  data() {
    ...
    // 部分 Android 设备不返回 safeAreaInsets，按 0 处理并保留原 50px 内容区。
    return {
      placeholderHeight: 0,
      safeBottomHeight: device.safeAreaInsets?.bottom || 0
    };
  },
  computed: {
    tabbarStyle() {
    ...
    hostManagedUnimpApp() {
      return isHostManagedUnimpApp();
    },
    // 54px 内容区只用于存在真实底部安全区的宿主管理 App，其他设备继续使用 50px。
    shouldUseHostManagedSafeBottom() {
      return this.hostManagedUnimpApp && this.safeAreaInsetBottom && this.safeBottomHeight > 0;
    }
  },
  watch: {
    updateChild() {
...
.u-tabbar {
  ...
  /* #ifdef APP-PLUS */
  // 与 xxt-layout 的安全区高度配套，padding 计入 54px 固定高度，避免图标文字整体下沉。
  .u-tabbar--host-managed-safe-bottom {
